...
int main(int argc, char ** argv) {
    std::cout << "Flash Attention 3 - Hopper Implementation" << std::endl;
    std::cout << "==========================================" << std::endl;
    ...
    for (int i = 0; i < device_count; i++) {
        auto props = at::cuda::getDeviceProperties(i);
        std::cout << "Device " << i << ": " << props->name
            << " (Compute " << props->major << "." << props->minor << ")" << std::endl;
    }

    std::cout << "\nFlash Attention library initialized successfully!" << std::endl;
    std::cout << "Maximum supported head dimension: " << get_max_headdim() << std::endl;

    // Minimal test: invoke flash attention forward pass
    std::cout << "\n--- Running minimal test ---" << std::endl;
    ...
    try {
        ...
        auto v = torch::randn({ batch_size, seqlen_k, num_heads, head_dim }, options);

        std::cout << "Calling mha_fwd..." << std::endl;

        // Call flash attention forward pass with minimal arguments
        auto result = mha_fwd(
            q, k, v,
            std::nullopt,  // k_new
            std::nullopt,  // v_new
        ...
        auto softmax_lse = std::get<1>(result);

        std::cout << "Output shape: [" << out.size(0) << ", " << out.size(1)
            << ", " << out.size(2) << ", " << out.size(3) << "]" << std::endl;
        std::cout << "LSE shape: [" << softmax_lse.size(0) << ", " << softmax_lse.size(1)
            << ", " << softmax_lse.size(2) << "]" << std::endl;

        // Simple sanity check
        auto out_cpu = out.cpu();
        bool has_nan = torch::isnan(out_cpu).any().item<bool>();
        bool has_inf = torch::isinf(out_cpu).any().item<bool>();
